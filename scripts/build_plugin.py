#!/usr/bin/env python3
"""Build and validate the distributable HumanToday plugin archive."""
...
DIST = ROOT / "dist"
SKILL_ROOT = ROOT / "skills" / "humantoday"
...
def validate_source(manifest: dict) -> None:
    ...
    for source in (SKILL_ROOT / "SKILL.md", SKILL_ROOT / "eval.md", ROOT / "assets" / "humantoday.png"):
        if not source.is_file():
            raise SystemExit(f"Missing package source: {source.relative_to(ROOT)}")


def build_plugin(manifest: dict) -> tuple[Path, Path]:
    plugin_root = DIST / "humantoday"
    if plugin_root.exists():
        shutil.rmtree(plugin_root)

    skill_root = plugin_root / "skills" / "humantoday"
    ...
    shutil.copy2(ROOT / "assets" / "humantoday.png", plugin_root / "assets" / "humantoday.png")
    ...
    archive = DIST / f"humantoday-plugin-{manifest['version']}.zip"
    if archive.exists():
        archive.unlink()
...
def validate_build(plugin_root: Path, archive: Path) -> None:
    expected = {
        ...
        "assets/humantoday.png",
        "skills/humantoday/SKILL.md",
        "skills/humantoday/eval.md",
        "LICENSE",
        "PRIVACY.md",
        "TERMS.md",
    }
    ...
    packaged_skill = plugin_root / "skills" / "humantoday" / "SKILL.md"
    packaged_eval = plugin_root / "skills" / "humantoday" / "eval.md"
